...
##Set my working directory as /Users/aelinadas/Desktop/01_001050895_AelinaDas/data
...
## [your code here][Aelina Das]
topGradRate <- select(df, X, Grad.Rate)
descTopGradRate <- arrange(topGradRate, desc(Grad.Rate))
filter(descTopGradRate, Grad.Rate >= 80)
...
## [your code here]
dfmaxmin <- group_by(df,Private)
dfmaxmin <- summarise(dfmaxmin,max=max(Outstate),min=min(Outstate))
dfmaxmin

collegemaxmin <- group_by(college,Private)
collegemaxmin <- summarise(collegemaxmin,max=max(Outstate),min=min(Outstate))
collegemaxmin
